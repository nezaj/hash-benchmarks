/**
 * A faster hashing implementation inspired by djb2/fnv1a algorithms.
 * Optimized for speed while maintaining good distribution.
 * 
 * @param {any} input - Value to hash
 * @returns {string} - Hash in hex format
 */
export default function fastHash(input) {
  ...
  if (Array.isArray(input)) {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      const elementHash = fastHash(input[i]);
  ...
  if (typeof input === 'object') {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const keyHash = fastHash(key);
      hash ^= parseInt(keyHash, 16);
      hash *= 16777619;
      hash = hash >>> 0;

      // Hash the value recursively
      const valueHash = fastHash(input[key]);
